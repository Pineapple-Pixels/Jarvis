...
func BuildToolRegistry(
	...
	calendarClient CalendarProvider,
	gmailClient GmailProvider,
	todoistClient TodoistProvider,
	githubClient GitHubProvider,
	jiraClient JiraProvider,
	spotifyClient SpotifyProvider,
	_ any, // notionClient — reserved, not used by tools
	_ any, // obsidianVault — reserved, not used by tools
	skillWriter skills.SkillWriter,
	reminderMgr *ReminderManager,
) *ToolRegistry {
	r := NewToolRegistry()

	registerFinanceTools(r, financeUC)
	registerMemoryTools(r, memorySvc, embedder)
	registerCalendarTools(r, calendarClient)
	registerGmailTools(r, gmailClient)
	registerTodoistTools(r, todoistClient)
	registerGitHubTools(r, githubClient)
	registerJiraTools(r, jiraClient)
	registerSpotifyTools(r, spotifyClient)
	registerSkillTools(r, skillWriter)
	registerUtilityTools(r, reminderMgr)

	return r
}

func registerFinanceTools(r *ToolRegistry, financeUC *FinanceUseCase) {
	if financeUC == nil {
		return
	}
...
func registerMemoryTools(r *ToolRegistry, memorySvc service.MemoryService, embedder service.Embedder) {
	if memorySvc == nil {
		return
	}
...
func registerCalendarTools(r *ToolRegistry, calendarClient CalendarProvider) {
...
func registerGmailTools(r *ToolRegistry, gmailClient GmailProvider) {
	if gmailClient == nil {
		return
	}
...
func registerTodoistTools(r *ToolRegistry, todoistClient TodoistProvider) {
...
func registerGitHubTools(r *ToolRegistry, githubClient GitHubProvider) {
	if githubClient == nil {
		return
	}
	...
	r.RegisterWithLimit(domain.ToolDefinition{
		...
		issue, err := githubClient.CreateIssue(
			inputString(input, "owner"),
			inputString(input, "repo"),
...
func registerJiraTools(r *ToolRegistry, jiraClient JiraProvider) {
	if jiraClient == nil {
		return
	}
...
func registerSpotifyTools(r *ToolRegistry, spotifyClient SpotifyProvider) {
...
func registerSkillTools(r *ToolRegistry, skillWriter skills.SkillWriter) {
	if skillWriter == nil {
		return
	}
...
func registerUtilityTools(r *ToolRegistry, reminderMgr *ReminderManager) {
	r.Register(domain.ToolDefinition{
		Name:        "get_usage",
